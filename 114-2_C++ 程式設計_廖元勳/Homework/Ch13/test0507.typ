...
struct User { string name; }; \/\/ 參數用 \* 接收地址void
changeName(User\* u) { u-\>name = "Alice"\; \/\/ 使用 -\> 存取成員 } int
main() { User person = {"Bob"}; \/\/ 呼叫時必須加 & 取得地址
changeName(&person); }
...
struct User { string name; }; \/\/ 參數用 & 宣告為引用void
changeName(User& u) { u.name = "Alice"\; \/\/ 使用 .
存取成員，像操作一般變數 } int main() { User person = {"Bob"}; \/\/
呼叫時直接傳入物件，不需加符號 changeName(person); }
